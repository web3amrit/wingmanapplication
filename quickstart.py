...
async def convert_image(file: UploadFile):
    logging.info(f"Converting image: {file.filename}")
    try:
    ...
    logging.info(f"Image conversion completed: {file.filename}")
    return UploadFile(
...
async def upload_image_to_blob(file: UploadFile):
    ...
    logging.info(f"Checking if the blob: {blob_name} already exists")
    try:
    ...
    logging.info(f"Starting to upload file: {file.filename} to blob: {blob_name}")
    block_list = []
    try:
        ...
        logging.info(f"Successfully uploaded file: {file.filename} to blob: {blob_name}")
...
async def create_upload_file(file: UploadFile = File(...)):
    ...
    logging.info(f"Received file: {file.filename} for upload and conversion")
    try:
    ...
    logging.info(f"Sending processed image data to DAI service")
    try:
